...
;; (define (do-line input right down)
;;   ;;  (if (not (eof-object? input))
;;   (if (eof-object? input)
;;       (display "The End")
;;       (do-line (read-line file) 0 0)))

;;(format #t ":: ~s :: ~s :: ~d\n" line (string-ref line right) right)
;;  (if (eof-object? line)
;;      (display "The End")

(define (do-line port right down)
  (define line (read-line port))
  (if (not (eof-object? line))
      (begin
        (if (eq? (string-ref line right) #\#)
            (set! cnt (+ cnt 1)))

        ;; print stats 
        (format #t ":: ~s :: ~s :: right:~d down:~d cnt:~d\n" line (string-ref line right) right down cnt)

        ;; if we're at char 30 (eol), 3 further is not 3, but 2 since
        ;; index starts at 0. Could solve this with a calculation, but
        ;; hey, it works for now.
        (cond ((= right 30)
               (do-line port 2 (+ down 1)))
              ((= right 29)
               (do-line port 1 (+ down 1)))
              ((= right 28)
              (do-line port 0 (+ down 1)))
              (else
               (do-line port (+ right 3) (+ down 1)))))))
